...
class NeuroMLCAP(object):
    ...
    def run(self):
        ...
        self.nml_doc = read_neuroml2_file(self.cell_file)   # type: neuroml.NeuroMLDocument
        ...
        if self.cfg['default']["fi_curves"] is True:
            ...
            if len(self.cfg['fi_curves']['currents']) == 0:
                ...
                self.sim_counter = 0
                for cr in currents:
                    simid, lems_file = self.generate_step_current_sim(segment_id=0, current_nA=cr)
                    self.recorder[simid] = {
                        "simfile": lems_file,
                        "segment": "0",
                        "current": cr
    ...
    def __get_segments_to_record(self):
        ...
        self.unbranched_segment_groups = self.cell_obj.get_segment_groups_by_substring("", unbranched=True)
        # pick N
    ...
    def generate_step_current_sim(self, current_nA: str, segment_id: str):
        ...
        net = net_doc.add(neuroml.Network, id="network", type="networkWithTemperature",
                          ...
                          validate=False)
        pop = net.add(neuroml.Population, id=f"population_of_{self.cell_obj.id}",
                      component=self.cell_obj.id, type="populationList", size=1, validate=False)
        ...
        for s in self.recorded_segments.keys():
            ls.add_column_to_output_file("output_file", f"v_cell_0_{s}", f"{pop.id}/0/{self.cell_obj.id}/{s}/v")
